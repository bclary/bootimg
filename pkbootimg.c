...
int usage(void)
{
    fprintf(stderr,"usage: pkbootimg\n"
            "       --kernel <filename>\n"
            "       --kernel-addr <address>\n"
            "       --ramdisk <filename>\n"
...
int main(int argc, char **argv)
{
    ...
    SHA_CTX ctx;
    uint8_t* sha;
    unsigned kernel_addr  = 0x0;
    unsigned ramdisk_addr = 0x0;
    unsigned second_addr  = 0x0;
    unsigned tags_addr    = 0x0;
    ...
    while(argc > 0){
        ...
        if(!strcmp(arg, "--output") || !strcmp(arg, "-o")) {
            ...
            cmdline = val;
        } else if(!strcmp(arg, "--kernel-addr")) {
            kernel_addr = strtoul(val, 0, 16);
        } else if(!strcmp(arg, "--ramdisk-addr")) {
            ramdisk_addr = strtoul(val, 0, 16);
        } else if(!strcmp(arg, "--second-addr")) {
            second_addr = strtoul(val, 0, 16);
    ...
    if (kernel_addr == 0x0 || ramdisk_addr == 0x0 ||
        second_addr == 0x0 || tags_addr    == 0x0) {
      fprintf(stderr,"error: Missing address. All address arguments must be specified.\n");
      return usage();
    }
